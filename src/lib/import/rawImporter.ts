...
async function getRawRecords(
  ...
  const cmd = `mysql -h${config.host} -P${config.port} -u${config.user} -p'${config.password}' ${tempDbName} -e "SELECT * FROM ${table}" --batch`
...
async function rawInsert(
  ...
  const cmd = `mysql -h${config.host} -P${config.port} -u${config.user} -p'${config.password}' ${config.database} -e "${sql.replace(/"/g, '\\"')}"`

When using the breed management page to add a new breed, it does not allow you to add a new breed when you set the avgcost to a value that is not a multiple of 5. Any value for the avgcost field is valid, except $0 or null, but $1, or $71, or $1001
...
✔️ Customer History
✔️ Database Backup (note sidebar icon doesn't work from this page but header does) sidebar hamburger menu not working on the backup page.
...
❌ cannot update existing breed avgtime, goes to date format
